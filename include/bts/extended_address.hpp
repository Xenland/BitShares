...
FC_REFLECT( bts::extended_address, (addr) )

namespace fc 
{ 
   void to_variant( const bts::extended_address& var,  fc::variant& vo );
   void from_variant( const fc::variant& var,  bts::extended_address& vo );
}

namespace std
{
   template<>
   struct hash<bts::extended_address> 
   {
       public:
         size_t operator()(const bts::extended_address &a) const 
         {
            size_t s;
            fc::sha1::encoder enc;
            fc::raw::pack( enc, a );
