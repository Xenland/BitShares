...
namespace fc 
{ 
   void to_variant( const bts::address& var,  fc::variant& vo );
   void from_variant( const fc::variant& var,  bts::address& vo );
}

namespace std
{
   template<>
   struct hash<bts::address> 
   {
       public:
         size_t operator()(const bts::address &a) const 
         {
            size_t s;
